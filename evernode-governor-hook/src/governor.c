                ...
                if (op_type != OP_NONE)
                {
                    ...
                    if (op_type == OP_STATUS_CHANGE)
                    {
                        ...
                        else if (candidate_type == DUD_HOST_CANDIDATE)
                        {
                            ...
                            {
                                // TODO: Emit evnDudHostRemove transaction to registry so it could handle the the host removal.
                            }

                            if (vote_status != STATUS_ACTIVE && state_foreign_set(0, 0, SBUF(STP_CANDIDATE_ID), FOREIGN_REF) < 0)
                                rollback(SBUF("Evernode: Could not set state for piloted mode candidate."), 1);
