        ...
        if (!BUFFER_EQUAL_20(hook_accid, account_field))
        {
            ...
            if (memos_len)
            {
                ...
                if (memo_lookup > 0)
                {
                    ...
                    // ASSERT_FAILURE_MSG >> Memo type is invalid.
                    ASSERT(EQUAL_CANDIDATE_PROPOSE_REF(type_ptr1, type_len1));

                    // ASSERT_FAILURE_MSG >> Memo format should be hex.
                    ASSERT(EQUAL_FORMAT_HEX(format_ptr1, format_len1))
                }
                ...
                if (op_type != OP_NONE)
                {
                    ...
                    // ASSERT_FAILURE_MSG >> Memo format should be hex.
                    ASSERT(EQUAL_FORMAT_HEX(format_ptr, format_len));
                    ...
                    if (op_type == OP_INITIALIZE)
                    {
                        ...
                        ASSERT(data_len == (4 * ACCOUNT_ID_SIZE));
                    ...
                    else if (op_type == OP_PROPOSE)
                    {
                        ...
                        ASSERT(BUFFER_EQUAL_32((data_ptr1 + CANDIDATE_PROPOSE_UNIQUE_ID_MEMO_OFFSET), unique_id));
                    ...
                    else if (op_type == OP_HOOK_UPDATE)
                    {
                        ...
                        ASSERT(BUFFER_EQUAL_32(data_ptr, &governance_info[ELECTED_PROPOSAL_UNIQUE_ID_OFFSET]));
                    ...
                    else if (op_type == OP_GOVERNANCE_MODE_CHANGE)
                    {
                        ...
                        ASSERT(*(data_ptr) > governance_info[GOVERNANCE_MODE_OFFSET]);
                    ...
                    else if (op_type == OP_DUD_HOST_REPORT)
                    {
                        ...
                        // ASSERT_FAILURE_MSG >> Generated candidate hash is not matched with provided.
                        ASSERT(BUFFER_EQUAL_32(data_ptr, unique_id));
