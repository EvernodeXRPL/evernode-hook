...
const uint32_t DEF_GOVERNANCE_ELIGIBILITY_PERIOD = 86400; // 1 Day in seconds.
const uint32_t DEF_CANDIDATE_LIFE_PERIOD = 604800;        // 1 Week in seconds.
const uint32_t DEF_CANDIDATE_ELECTION_PERIOD = 172800;    // 2 Days in seconds.
...
const uint8_t DEF_HOST_REPUTATION_THRESHOLD = 200;
...
uint8_t HEARTBEAT_FUND_PAYMENT[424] = {
    0x12, 0x00, 0x00,                   // transaction_type(ttPAYMENT)
    0x22, 0x80, 0x00, 0x00, 0x00,       // flags(tfCANONICAL)
    0x23, 0x00, 0x00, 0x00, 0x00,       // TAG_SOURCE
    0x24, 0x00, 0x00, 0x00, 0x00,       // sequence(0)
...
#define PREPARE_HEARTBEAT_FUND_PAYMENT_TX(evr_amount, to_address, tx_ref)     \
    {                                                                         \
        ...
        COPY_16BYTES((buf_out + 214), UPDATE_REWARD_POOL);                    \
        COPY_2BYTES((buf_out + 214 + 16), UPDATE_REWARD_POOL + 16);           \
        COPY_BYTE((buf_out + 214 + 18), UPDATE_REWARD_POOL + 18);             \
        COPY_32BYTES((buf_out + 274), tx_ref);                                \
