...
int64_t hook(uint32_t reserved)
{
    ...
    // Future Purpose : Hook Hash of the next Hook (POSITION 1)
    // NOTE : We can use hook_skip function, if we want to skip a particular hook. Hence for that we can use this hook hash
    uint8_t hook_pos_one_hash_ptr[32];
    hook_hash(hook_pos_one_hash_ptr, 32, 1);
    ...
    if ((reserved == STRONG_HOOK || reserved == AGAIN_HOOK) && (txn_type == ttPAYMENT || txn_type == ttNFT_ACCEPT_OFFER))
    {
        ...
        else if (reserved == STRONG_HOOK && txn_type == ttPAYMENT)
        {
            ...
            if (is_xrp)
            {
                ...
                if (is_initialize)
                {
                    hook_skip(hook_pos_one_hash_ptr, 32, 0);
    ...
    accept(SBUF("Evernode: Transaction is not handled in Hook Position 0."), 0);
