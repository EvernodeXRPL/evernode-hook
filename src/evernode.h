...
#define PREPARE_EPOCH_REWARD_INFO(reward_info, moment_base_idx, moment_size, is_heartbeat, reward_pool_amount_ref, reward_quota_ref)                              \
    {                                                                                                                                                             \
        ...
        uint32_t moment_reward_amount = EPOCH_REWARD_AMOUNT / epoch_duration;                                                                                     \
        ...
        {                                                                                                                                                         \
            ...
            UINT32_TO_BUF(&reward_info[CUR_MOMENT_ACTIVE_HOST_COUNT_OFFSET], (is_heartbeat ? 1 : 0));                                                             \
                                                                                                                                                                  \
            /* Reward pool amount is less than the minimum needed reward amount for the moment, Increment the epoch. And add the remaining to next epoch pool. */ \
            if (float_compare(reward_pool_amount_ref, float_set(0, moment_reward_amount), COMPARE_LESS) == 1)                                                     \
            {                                                                                                                                                     \
                /* If the current epoch is < 10 increment otherwise skip. */                                                                                      \
                ...
                {                                                                                                                                                 \
                    ...
                    /* When epoch incremented by 1, epoch_duration doubles and reward amount halves. */                                                           \
                    epoch_duration = epoch_duration * 2;                                                                                                          \
                    moment_reward_amount = moment_reward_amount / 2;                                                                                              \
                }                                                                                                                                                 \
                else if (epoch == EPOCH_COUNT)                                                                                                                    \
                {                                                                                                                                                 \
                    moment_reward_amount = 0;                                                                                                                     \
                }                                                                                                                                                 \
            }                                                                                                                                                     \
        }                                                                                                                                                         \
        ...
        /* Use float division only if modulo is not zero to reduce floating point conplications. */                                                               \
        if (prev_moment_active_host_count == 0)                                                                                                                   \
            reward_quota_ref = float_set(0, 0);                                                                                                                   \
        else if (moment_reward_amount % prev_moment_active_host_count == 0)                                                                                       \
            reward_quota_ref = float_set(0, (moment_reward_amount / prev_moment_active_host_count));                                                              \
        else                                                                                                                                                      \
            reward_quota_ref = float_divide(float_set(0, moment_reward_amount), float_set(0, prev_moment_active_host_count));                                     \
    }
