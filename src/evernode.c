...
{
    if (reserved == STRONG_HOOK || reserved == AGAIN_HOOK)
    {
        ...
        if (txn_type == ttPAYMENT || txn_type == ttNFT_ACCEPT_OFFER)
        {
            ...
            if (txn_type == ttPAYMENT)
            {
                ...
                else
                {
                    ...
                    accept(SBUF("Evernode: IOU transaction."), 0);
                }
            }
            else if (txn_type == ttNFT_ACCEPT_OFFER)
            {
                ...
                accept(SBUF("Evernode: NFT offer accept transaction."), 0);
            }
        }
    }

    accept(SBUF("Evernode: Transaction type not handled."), 0);
