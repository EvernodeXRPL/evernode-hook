...
int64_t hook(uint32_t reserved)
{
    ...
    ASSERT(hook_param(SBUF(state_hook_accid), SBUF(PARAM_STATE_HOOK)) >= 0);
    ...
    if (memos_len)
    {
        ...
        if (op_type != OP_NONE)
        {
            ...
            ASSERT(op_type == OP_HOST_REBATE || EQUAL_FORMAT_HEX(format_ptr, format_len));
            ...
            if (op_type == OP_HOST_DE_REG)
            {
                ...
                ASSERT(BUFFER_EQUAL_32(data_ptr, (host_addr + HOST_TOKEN_ID_OFFSET)));
