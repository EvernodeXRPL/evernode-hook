...
int64_t hook(uint32_t reserved)
{
    ...
    if (hook_param(SBUF(state_hook_accid), SBUF(PARAM_STATE_HOOK_KEY)) < ACCOUNT_ID_SIZE)
    ...
    if (txn_type == ttPAYMENT)
    {
        ...
        if (!BUFFER_EQUAL_20(hook_accid, account_field))
        {
            ...
            if (op_type != OP_NONE)
            {
                ...
                if (op_type != OP_HEARTBEAT && event_data_len < 0)
                ...
                if (op_type == OP_VOTE)
                {
                    ...
                    else
                    {
                        ...
                        {
                            if (cur_moment == voted_moment && *(event_data + CANDIDATE_VOTE_VALUE_PARAM_OFFSET) == CANDIDATE_SUPPORTED && voted_flag == 1)
