...
if [ -z "$arg1" ]; then # If 1st param is empty, Create a new instance.
    ...
    ACCOUNT_DATA_DIR=$data_dir $(which node) $account_setup || echo "Accounnt setup faild."
    arg1=$(ls -t $hook_data_dir/ | head -1)
    CONFIG_PATH=$hook_data_dir/$arg1/$config WASM_PATH=$wasm_path $(which node) $hook_setup || echo "Hook setup faild."
